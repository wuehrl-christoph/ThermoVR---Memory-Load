...
public class StudySetupManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // load csv/scene sequence coresponds to pid
        loadCSV();
        // print ID, counter + scene sequence
        setupButton();
    }
    ...
    {
        if(int.Parse(number) == 1 || int.Parse(number) == 3)
        {
            return 1;
        }
        else if(int.Parse(number) == 2 || int.Parse(number) == 4)
        {
            return 3;
        }
        else
